...
int main(int argc, char *argv[]) {
  if (argc < 2) {
    std::cerr << "Usage: ps1xRuntime <ps1_elf_or_iso> [disc_image]\n"
              << "       ps1xRuntime <disc_image> <elf_boot_exe>\n";
    return 1;
  }

  // Support two argument styles:
  //   1 arg : input_path is the ELF (disc auto-detected from .boot.exe name)
  //   2 args: argv[1]=disc image, argv[2]=ELF boot exe
  std::string input_path;
  std::string explicit_disc_path;
  if (argc > 2) {
    explicit_disc_path = argv[1];
    input_path = argv[2];
  } else {
    input_path = argv[1];
  }
  ...
  cdromCtrl.setInterruptCallback([&bios](uint8_t intType) {
    bios.triggerCdromEvent(intType);
  });
  ...
  if (!explicit_disc_path.empty()) {
    ...
    if (fs.loadDisc(explicit_disc_path)) {
      ...
      fmt::print(stderr,
                 "[CDROM] WARNING: Could not mount disc image: {}\n",
                 explicit_disc_path);
    }
  } else {
  ...
  while (running) {
    ...
    if (frameCount == 200) {
      ...
      fmt::print("[DEBUG] Frame {}: GPUSTAT=0x{:08X} displayEnabled={} "
                 "dispArea=({},{}) dispRange=({},{},{},{}) "
                 "hres_bits={}\n",
                 frameCount, stat, gpu.isDisplayEnabled(),
                 dispX, dispY, rx1, rx2, ry1, ry2,
                 (stat >> 17) & 3);
      // Dump VRAM to PPM
