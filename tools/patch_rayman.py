...
NEW_VSYNC_WAIT = (
    ...
    '        fflush(stderr);\n'
    '    }\n'
    '    // Phase 3.2 moved triggerVBlankEvent from the VBlank thread into\n'
    '    // hle_libetc_VSync via HleConfig::deliverVBlankEvent.  Rayman\n'
    '    // does NOT call libetc_VSync — it uses this custom func_801B954C\n'
    '    // wait — so without firing deliverVBlankEvent here, PsyQ events\n'
    '    // never trigger and callback-mode handlers (RootCounter 1 etc.)\n'
    '    // never run.  Drain the vblankPending flag exactly as hle_VSync\n'
    '    // would: exchange acq_rel, fire delivery once if observed true.\n'
    '    uint32_t target = (uint32_t)(ctx->r4);\n'
    '    auto &vsync = ps1::psyq::psyq_state().vsyncCounter;\n'
    '    auto &pending = ps1::psyq::psyq_state().vblankPending;\n'
    '    // Wall-clock deadline of 1.5 s (~90 frames at 60 Hz) — bigger\n'
    '    // than any realistic single VSync(N) wait but bounded so a\n'
    '    // dead VBlank thread cannot deadlock the game.  Iteration cap\n'
    '    // alone (100 × 1 ms) was timing out short of the target when\n'
    '    // the game called VSync(current+N) with N > 6 — boot init then\n'
    '    // thought N frames elapsed but counter was still mid-advance,\n'
    '    // putting downstream code into a slow-progress mode (~4 calls/s\n'
    '    // instead of 60).\n'
    ...
    '    }\n'
    '    if (pending.exchange(false, std::memory_order_acq_rel) && ctx->bios)\n'
    '        ctx->bios->triggerVBlankEvent();\n'
    '    if (ctx->bios) ctx->bios->drainPendingCallbacks();\n'
