...
std::string InstructionEmitter::reg(uint8_t r) {
    if (r == 0) return "(int32_t)0";  // $zero is always 0
...
std::string InstructionEmitter::emitALU(const Instruction& inst) const {
    ...
    switch (inst.id) {
        ...
        case InstrId::ADDI:
        case InstrId::ADDIU:
            return fmt::format("{} = (int32_t)({} + {});", dst, s32(reg(inst.rs)), inst.imm16);
        case InstrId::ANDI:
        ...
        case InstrId::XORI:
            return fmt::format("{} = {} ^ 0x{:04X};", dst, reg(inst.rs),
                               static_cast<uint16_t>(inst.imm16));
        case InstrId::SLTI:
            return fmt::format("{} = ({} < {}) ? 1 : 0;", dst, s32(reg(inst.rs)), inst.imm16);
        case InstrId::SLTIU:
    ...
    switch (inst.id) {
        case InstrId::MULT:
            return fmt::format("{{ int64_t result = (int64_t){} * (int64_t){}; "
                               ...
                               s32(rs), s32(rt));
        case InstrId::MULTU:
            return fmt::format("{{ uint64_t result = (uint64_t){} * (uint64_t){}; "
                               "ctx->lo = (int32_t)result; ctx->hi = (int32_t)(result >> 32); }}",
                               u32(rs), u32(rt));
        case InstrId::DIV:
            return fmt::format("if ({} != 0) {{ ctx->lo = {} / {}; ctx->hi = {} % {}; }}",
                               s32(rt), s32(rs), s32(rt), s32(rs), s32(rt));
        case InstrId::DIVU:
        ...
        default:
            return fmt::format("// UNKNOWN MULDIV: {}", MipsDecoder::instrName(inst.id));
    }
}
...
std::string InstructionEmitter::emitLoad(const Instruction& inst) const {
    ...
    switch (inst.id) {
        ...
        case InstrId::LBU:
            return fmt::format("{} = (uint8_t)MEM_READ8(ctx, {} + {});", rt, s32(base), offset);
        case InstrId::LH:
            return fmt::format("{} = MEM_READ16(ctx, {} + {});", rt, s32(base), offset);
        case InstrId::LHU:
            return fmt::format("{} = (uint16_t)MEM_READ16(ctx, {} + {});", rt, s32(base), offset);
        case InstrId::LW:
            return fmt::format("{} = MEM_READ32(ctx, {} + {});", rt, s32(base), offset);
        case InstrId::LWL:
            return fmt::format("{} = DO_LWL(ctx, {}, {} + {});", rt, rt, s32(base), offset);
        case InstrId::LWR:
            return fmt::format("{} = DO_LWR(ctx, {}, {} + {});", rt, rt, s32(base), offset);
        default:
...
std::string InstructionEmitter::emitBranch(const Instruction& inst, uint32_t pc) const {
    auto rs = reg(inst.rs);
    auto rt = reg(inst.rt);
    ...
    switch (inst.id) {
        ...
        case InstrId::BGEZ:
            return fmt::format("if ({} >= 0) goto {};", s32(rs), target);
        case InstrId::BLTZAL:
            return fmt::format("ctx->r31 = 0x{:08X}; if ({} < 0) goto {};",
                               pc + 8, s32(rs), target);
        case InstrId::BGEZAL:
            return fmt::format("ctx->r31 = 0x{:08X}; if ({} >= 0) goto {};",
                               pc + 8, s32(rs), target);
        default:
            return fmt::format("// UNKNOWN BRANCH: {}", MipsDecoder::instrName(inst.id));
    }
}

// ─── Jump Emitter ────────────────────────────────────────

std::string InstructionEmitter::emitJump(const Instruction& inst, uint32_t pc) const {
    switch (inst.id) {
        case InstrId::J:
        ...
        case InstrId::JALR:
            return fmt::format("ctx->r{} = 0x{:08X}; CALL_INDIRECT(ctx, {});",
                               inst.rd, pc + 8, reg(inst.rs));
        default:
...
std::string InstructionEmitter::emitGTEMove(const Instruction& inst) const {
    switch (inst.id) {
        ...
        case InstrId::LWC2:
            return fmt::format("ctx->cop2d[{}] = MEM_READ32(ctx, {} + {});",
                               inst.rt, s32(reg(inst.rs)), inst.imm16);
        case InstrId::SWC2:
...
std::string InstructionEmitter::emitSystem(const Instruction& inst) const {
    switch (inst.id) {
        ...
        case InstrId::BREAK:
            return "BREAK(ctx);";
        default:
            return fmt::format("// UNKNOWN SYSTEM: {}", MipsDecoder::instrName(inst.id));
    }
}

// ─── Main Dispatch ───────────────────────────────────────

std::string InstructionEmitter::emitInstruction(const Instruction& inst, uint32_t pc) const {
    if (inst.isNOP()) return "// NOP";
    if (!inst.isValid()) return fmt::format("// INVALID: 0x{:08X}", inst.raw);
    ...
    switch (inst.category) {
        case InstrCategory::ALU:
            ...
            if (inst.id <= InstrId::SLTU) writesToZero = (inst.rd == 0);
            else writesToZero = (inst.rt == 0);
            break;
        case InstrCategory::Memory:
            if (inst.isLoad()) writesToZero = (inst.rt == 0);
            break;
    ...
    if (writesToZero) return "// NOP (writes to $zero)";
    ...
    switch (inst.category) {
        ...
        case InstrCategory::Memory: return inst.isStore() ? emitStore(inst) : emitLoad(inst);
        case InstrCategory::Branch: return emitBranch(inst, pc);
        case InstrCategory::Jump:   return emitJump(inst, pc);
        case InstrCategory::MulDiv: return emitMulDiv(inst);
        case InstrCategory::System: return emitSystem(inst);
        case InstrCategory::COP0:   return emitCOP0(inst);
        case InstrCategory::GTE: {
            // GTE commands vs register moves
            if (inst.id >= InstrId::GTE_RTPS && inst.id <= InstrId::GTE_NCCT) {
                return fmt::format("GTE_{}(ctx);", MipsDecoder::instrName(inst.id));
            }
            return emitGTEMove(inst);
        }
        case InstrCategory::Special:
            return fmt::format("// {}: 0x{:08X}", MipsDecoder::instrName(inst.id), inst.raw);
        default:
