...
static InstructionEmitter makeEmitter() {
    ...
    emitter.setFuncResolver([](uint32_t addr) -> std::string {
        if (addr == 0x80020000) return "GameUpdate";
        if (addr == 0x80030000) return "RenderFrame";
        return "";
    });
    return emitter;
}
...
TEST(InstructionEmitter, ALU_ADDIU) {
    auto emitter = makeEmitter();
    // ADDIU $sp, $sp, -32
    auto inst = MipsDecoder::decode(encI(0x09, 29, 29, static_cast<uint16_t>(-32)));
    auto code = emitter.emitInstruction(inst, 0x80010000);
...
TEST(InstructionEmitter, ALU_BitwiseOps) {
    ...
    // AND $v0, $a0, $a1
    auto code = emitter.emitInstruction(MipsDecoder::decode(encR(0, 4, 5, 2, 0, 0x24)), 0);
    EXPECT_NE(code.find("&"), std::string::npos);

    // OR
    code = emitter.emitInstruction(MipsDecoder::decode(encR(0, 4, 5, 2, 0, 0x25)), 0);
    EXPECT_NE(code.find("|"), std::string::npos);

    // XOR
    code = emitter.emitInstruction(MipsDecoder::decode(encR(0, 4, 5, 2, 0, 0x26)), 0);
    EXPECT_NE(code.find("^"), std::string::npos);
}
...
TEST(InstructionEmitter, Shifts) {
    ...
    // SLL $v0, $a0, 4
    auto code = emitter.emitInstruction(MipsDecoder::decode(encR(0, 0, 4, 2, 4, 0x00)), 0);
    EXPECT_NE(code.find("<<"), std::string::npos);
    EXPECT_NE(code.find("4"), std::string::npos);

    // SRL $v0, $a0, 5
    code = emitter.emitInstruction(MipsDecoder::decode(encR(0, 0, 4, 2, 5, 0x02)), 0);
    EXPECT_NE(code.find(">>"), std::string::npos);
}
...
TEST(InstructionEmitter, MulDiv) {
    ...
    // MULT $a0, $a1
    auto code = emitter.emitInstruction(MipsDecoder::decode(encR(0, 4, 5, 0, 0, 0x18)), 0);
    EXPECT_NE(code.find("ctx->lo"), std::string::npos);
    EXPECT_NE(code.find("ctx->hi"), std::string::npos);

    // MFLO $v0
    code = emitter.emitInstruction(MipsDecoder::decode(encR(0, 0, 0, 2, 0, 0x12)), 0);
    EXPECT_NE(code.find("ctx->r2"), std::string::npos);
    EXPECT_NE(code.find("ctx->lo"), std::string::npos);

    // DIV — should check for zero
    code = emitter.emitInstruction(MipsDecoder::decode(encR(0, 4, 5, 0, 0, 0x1A)), 0);
...
TEST(InstructionEmitter, LoadByte) {
    auto emitter = makeEmitter();
    auto code = emitter.emitInstruction(MipsDecoder::decode(encI(0x20, 4, 2, 0)), 0);
...
TEST(InstructionEmitter, UnalignedLoadStore) {
    ...
    auto code = emitter.emitInstruction(MipsDecoder::decode(encI(0x22, 4, 2, 0)), 0);
...
TEST(InstructionEmitter, BranchBNE) {
    auto emitter = makeEmitter();
    auto code = emitter.emitInstruction(MipsDecoder::decode(encI(0x05, 4, 5, 4)), 0x80010000);
    EXPECT_NE(code.find("!="), std::string::npos);
    EXPECT_NE(code.find("goto"), std::string::npos);
}
...
TEST(InstructionEmitter, BranchComparisons) {
    ...
    // BLEZ
    auto code = emitter.emitInstruction(MipsDecoder::decode(encI(0x06, 4, 0, 4)), 0x80010000);
    EXPECT_NE(code.find("<= 0"), std::string::npos);

    // BGTZ
    code = emitter.emitInstruction(MipsDecoder::decode(encI(0x07, 4, 0, 4)), 0x80010000);
    EXPECT_NE(code.find("> 0"), std::string::npos);
}
...
TEST(InstructionEmitter, COP0) {
    ...
    // MFC0 $v0, SR
    auto code = emitter.emitInstruction(MipsDecoder::decode(encR(0x10, 0x00, 2, 12, 0, 0)), 0);
    EXPECT_NE(code.find("cop0"), std::string::npos);
    EXPECT_NE(code.find("SR"), std::string::npos);

    // MTC0 $v0, SR
    code = emitter.emitInstruction(MipsDecoder::decode(encR(0x10, 0x04, 2, 12, 0, 0)), 0);
    EXPECT_NE(code.find("cop0"), std::string::npos);
}
...
TEST(InstructionEmitter, GTE_RegisterMoves) {
    ...
    // MFC2 $v0, r5
    auto code = emitter.emitInstruction(MipsDecoder::decode(encR(0x12, 0x00, 2, 5, 0, 0)), 0);
    EXPECT_NE(code.find("cop2d"), std::string::npos);
    EXPECT_NE(code.find("ctx->r2"), std::string::npos);

    // MTC2 $v0, r5
    code = emitter.emitInstruction(MipsDecoder::decode(encR(0x12, 0x04, 2, 5, 0, 0)), 0);
    EXPECT_NE(code.find("cop2d"), std::string::npos);
}
...
TEST(InstructionEmitter, EmitFunctionWithBranch) {
    ...
    if (func.instructions.size() > 3) func.isLabelTarget[3] = true;
    func.size = func.instructions.size() * 4;
