...
namespace mips {
    ...
    inline int16_t  getImm16(uint32_t instr)   { return static_cast<int16_t>(instr & 0xFFFF); }
    ...
    inline bool isJR_RA(uint32_t instr) {
        return getOpcode(instr) == OP_SPECIAL &&
               getFunction(instr) == FUNC_JR &&
               getRs(instr) == REG_RA;
    }

    /// Check if instruction is ADDIU $sp, $sp, -N (prologue)
    inline bool isStackPrologue(uint32_t instr) {
        return getOpcode(instr) == OP_ADDIU &&
               getRs(instr) == REG_SP &&
               getRt(instr) == REG_SP &&
...
private:
    ...
    // Helpers
    void addFunction(uint32_t addr, const std::string& name, FunctionSource source);
