...
private:
    ...
    // Raw ELF data kept alive for section data pointers
    std::vector<uint8_t> m_elfData;

    bool validatePS1Elf(uint16_t machine, uint8_t elfClass, uint8_t encoding);
    void extractSections();
    void extractSymbols();
    SectionType classifySectionType(const std::string& name, uint32_t type, uint32_t flags);
};
