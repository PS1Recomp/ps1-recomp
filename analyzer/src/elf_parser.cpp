...
bool ElfParser::load(const std::string& path) {
    ...
    if (!validatePS1Elf(reader.get_machine(), reader.get_class(), reader.get_encoding())) {
        return false;
    }
    ...
    if (!isInRAM(m_entryPoint)) {
        m_error = fmt::format(
            "Entry point 0x{:08X} is not in PS1 RAM range", m_entryPoint);
        return false;
    }
...
bool ElfParser::validatePS1Elf(uint16_t machine, uint8_t elfClass, uint8_t encoding) {
...
SectionType ElfParser::classifySectionType(
    const std::string& name, uint32_t type, uint32_t flags
) {
